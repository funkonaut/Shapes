...
function highlightMoves(piece, row, col) {
    ...
    moves.forEach(([r, c]) => {
        ...
        if (!targetPiece) {
            ...
            // Capturable piece (not a shield) and not same color
...
function capturePiece(event) {
    ...
    if (selectedPiece && targetDot.dataset.captureTarget) {
        ...
        // Check capture rules:
        // - Not capturing same color (already ensured by highlight)
        // - Pentagons can't capture
        ...
        if (targetPiece && targetPiece.color !== currentPlayer && targetPiece.shape !== 'pentagon') {
            ...
            console.log("Cannot capture: either it's not an opponent piece or it's a shield or same color.");
        }
    } else {
            ...
            while (steps < remainingRange) {
                ...
                if (targetPiece) {
                    if (targetPiece.color === color && targetPiece.shape === 'pentagon') {
                        ...
                        // Enemy piece, capturable if not pentagon and we are not a pentagon
                        // If we are a pentagon, we do not capture
                        if (!isPentagon) {
                            moves.push([tempRow, tempCol]);
                        }
                    }
                    // Stop in this direction after hit any piece (captured or not)
                    break;
                }

                // Empty cell: add move and consume a step
                moves.push([tempRow, tempCol]);
                steps++;
            }
        }
...
function getLinearMoves(state, row, col, range, color) {
    ...
    for (const [dr, dc] of directions) {
        ...
        while (steps < range) {
            ...
            if (targetPiece) {
                if (targetPiece.color === color && targetPiece.shape === 'pentagon') {
                    // Pass through same-color pentagon without using a step
                    continue;
                } else if (!isPentagon && targetPiece.color !== color && targetPiece.shape !== 'pentagon') {
                    // Capturable enemy piece if not a pentagon
                    if (!isPentagon) {
                        moves.push([currentRow, currentCol]);
                    }
                }
                // Stop after hitting any piece (captured or block)
                break;
            }
        ...
        while (steps < range) {
            ...
            if (targetPiece) {
                if (targetPiece.color === color && targetPiece.shape === 'pentagon') {
                    ...
                    // Capturable enemy piece if not pentagon
                    moves.push([currentRow, currentCol]);
                }
